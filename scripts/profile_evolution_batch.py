...
# -*- coding: utf-8 -*-
"""
Batch-eval profile evolution experiment:
- Keeps SFT/GRPO unchanged.
- Evolves objective_weights via outcome-guided accuracy only.
- Each generation evaluates all candidates on the same eval set (mean reward).
"""
...
def maybe_inject_elite(
    population: List[Dict[str, float]],
    elite: Dict[str, float] | None,
    pop_size: int,
) -> List[Dict[str, float]]:
    if elite is None:
    ...
    def _dist(a: Dict[str, float], b: Dict[str, float]) -> float:
        ...
        return s
    if any(_dist(p, elite) < 1e-9 for p in population):
        return population[:pop_size]
    out = population[:pop_size]
    if len(out) < pop_size:
        out.append(deepcopy(elite))
    else:
        out[-1] = deepcopy(elite)
    return out
...
def build_llm_prompt(
    ...
    return (
        ...
        "Trajectory (recent candidate evaluations):\n"
        f"- recent_candidates: {json.dumps(history, ensure_ascii=True)}\n\n"
...
def main() -> None:
    ...
    print(
        ...
        f"k_reasoning={args.k_reasoning} samples_per_gen=eval_size"
    )
    ...
    eval_n = min(len(indices), args.eval_size)
    ...
    print(f"[profile-evo] eval_samples={len(eval_chats)} generations={gen_count} (batch eval)")
    ...
    llm_logs: List[dict] = []
    start_gen = 0
    best_so_far_reward: float | None = None
    best_so_far_profile: Dict[str, float] | None = None
        ...
        if progress_records:
            ...
            for r in progress_records:
                ...
                if r.get("best_reward") is not None:
                    br = float(r["best_reward"])
                    best_rewards.append(br)
                    best_sample_rewards.append(br)
                if r.get("best_profile") is not None:
                    bp = dict(r["best_profile"])
                    best_profiles.append(bp)
                if br is not None and bp is not None:
                    best_sample_history.append(
                        {"generation": r.get("generation"), "best_reward": br, "best_weights": dict(bp)}
                    )
                if br is not None and (best_so_far_reward is None or br > best_so_far_reward):
                    best_so_far_reward = br
                    best_so_far_profile = deepcopy(bp) if bp is not None else None
                llm_logs.append(r)
    ...
    for gen in range(start_gen, gen_count):
        ...
        print("=" * 60)
        print(
            f"[gen {gen}] start  (pop={args.population_size}, samples={len(eval_chats)}, "
        ...
        best_sample_rewards.append(best_reward)
        history_candidates = []
        for p in population:
            history_candidates.append(
                {"reward": fitness_map[id(p)], "weights": dict(p), "source": "population"}
            )
        for c in candidates:
            history_candidates.append(
                {"reward": fitness_map[id(c)], "weights": dict(c), "source": "llm"}
            )
        best_sample_history.append(
            {
                "generation": gen,
                "best_reward": best_reward,
                "best_weights": dict(best_profile),
                "candidates": history_candidates,
            }
        )
        llm_entry["best_reward"] = best_reward
        llm_entry["best_profile"] = best_profile
        llm_entry["sample_best_mean"] = best_reward
        llm_entry["sample_best_count"] = len(eval_chats)
        llm_logs.append(llm_entry)
        if best_so_far_reward is None or best_reward > best_so_far_reward:
            best_so_far_reward = best_reward
            best_so_far_profile = deepcopy(best_profile)
        ...
        if llm_only_now:
            ...
            population = maybe_inject_elite(population, best_so_far_profile, args.population_size)
